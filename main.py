...
def print_banner(output_dir: str, ffmpeg_status: bool):
    ...
    status_ffmpeg = (
        f"{Colors.GREEN}Disponible (Máxima resolución + Conversiones activas){Colors.END}"
        if ffmpeg_status
        else f"{Colors.YELLOW}No instalado (Descargas nativas activas){Colors.END}"
    )
...
def show_video_details(info: dict):
    ...
    print(
        f"{Colors.BOLD}Duración  :{Colors.END} {format_duration(info.get('duration'))}"
    )
    vistas = info.get("view_count")
    print(
        f"{Colors.BOLD}Vistas    :{Colors.END} {vistas:,}"
        if vistas
        else f"{Colors.BOLD}Vistas    :{Colors.END} N/A"
    )
...
def interactive_mode(downloader: YouTubeDownloader):
    ...
    while True:
        ...
        print(
            f"  {Colors.GREEN}2.{Colors.END} Descargar Video (Elegir Resolución: 1080p, 720p, etc.)"
        )
        print(
            f"  {Colors.GREEN}3.{Colors.END} Descargar solo Audio (MP3, M4A, WAV, FLAC)"
        )
        print(
            f"  {Colors.GREEN}4.{Colors.END} Descargar Lista de Reproducción (Playlist)"
        )
        print(
            f"  {Colors.GREEN}5.{Colors.END} Descargar Miniatura del Video (Thumbnail)"
        )
        ...
        if choice == "8":
            new_path = input(
                "\nIngresa la nueva ruta para guardar las descargas: "
            ).strip()
            if new_path:
                downloader.output_dir = os.path.abspath(new_path)
                os.makedirs(downloader.output_dir, exist_ok=True)
                print(
                    f"{Colors.GREEN}✓ Carpeta actualizada a: {downloader.output_dir}{Colors.END}"
                )
        ...
        try:
            ...
            if choice == "1":
                ...
                print(
                    f"{Colors.BOLD}Iniciando descarga en máxima calidad...{Colors.END}"
                )
            ...
            # 2. Video con resolución personalizada
                ...
                res_choice = input(
                    f"\n{Colors.BOLD}Elige resolución [1-{len(resolutions)}]: {Colors.END}"
                ).strip()
                selected_res = None
                if res_choice.isdigit() and 1 <= int(res_choice) <= len(resolutions):
                    ...
                    print(
                        f"{Colors.YELLOW}[!] Opción no válida. Se usará la mejor resolución disponible.{Colors.END}"
                    )
            ...
            # 3. Descargar solo audio
                ...
                show_video_details(info)
                print(
                    f"{Colors.BOLD}Iniciando descarga de audio ({audio_fmt.upper()})...{Colors.END}"
                )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Descargador avanzado de YouTube en Python"
    )
    parser.add_argument("url", nargs="?", help="URL del video o lista de reproducción")
    parser.add_argument(
        "-a", "--audio", action="store_true", help="Descargar únicamente el audio"
    )
    parser.add_argument(
        "--format",
        type=str,
        default="mp3",
        help="Formato de audio (mp3, m4a, wav, flac)",
    )
    parser.add_argument(
        "-r", "--resolution", type=str, help="Resolución (ej. 1080p, 720p, 480p)"
    )
    parser.add_argument(
        "-p",
        "--playlist",
        action="store_true",
        help="Indica que la URL es una lista de reproducción",
    )
    parser.add_argument(
        "-t", "--thumbnail", action="store_true", help="Descargar solo la miniatura"
    )
    parser.add_argument(
        "-s", "--subtitles", action="store_true", help="Descargar subtítulos"
    )
    parser.add_argument(
        "-o", "--output", type=str, default="downloads", help="Carpeta de destino"
    )
...
def main():
    ...
    if args.thumbnail:
        ...
        downloader.download_playlist(
            url, audio_only=args.audio, resolution=args.resolution
        )
    elif args.audio:
